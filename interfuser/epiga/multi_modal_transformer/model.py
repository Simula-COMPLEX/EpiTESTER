...
world_to_ix = {'npc_vertical': 0, 'npc_horizontal': 1, 'npc_behavior': 2, 'pedestrian_vertical': 3,
               'pedestrian_horizontal': 4,
               'pedestrian_direction_x': 5, 'pedestrian_direction_y': 6, 'pedestrian_speed': 7, 'weather_sun_angle': 8,
               'weather_fog_density': 9}

# model_config = GlobalConfig(root_dir='/home/complexse/workspace/Chengjie/epiga-project/multi_modal_transformer/file', setting='all')
model_config = GlobalConfig()
model_config.use_target_point_image = bool(1)
model_config.n_layer = 4
model_config.use_point_pillars = bool(0)
model_config.backbone = 'transFuser'
if bool(0):
    index_bev = model_config.detailed_losses.index("loss_bev")
    model_config.detailed_losses_weights[index_bev] = 0.0
...
class Transfuser(nn.Module):
    ...
    def __init__(self, config=model_config):
        ...
        self.image_encoder = ImageCNN(normalize=True)
        ...
        self.lidar_encoder = LidarEncoder(in_channels=in_channels)

        self.embeddings = nn.Embedding(num_embeddings=len(world_to_ix.values()), embedding_dim=48)
        self.attn = Attn()

        self.fc_l = nn.Linear(10, 10)
    ...
    def forward(self, image, lidar, var_name, var_value):
        """
        Image + LiDAR feature fusion using transformers
        Args:
            :param var_value:
            :param image: list of input images
            :param lidar: list of input LiDAR BEV
            :param var_name:
        """
        ...
        # print('transformer 1:', image_features.shape, lidar_features.shape)
        """
        ...
        # print('transformer 2:', image_features.shape, lidar_features.shape)
        ...
        # print('transformer 3:', image_features.shape, lidar_features.shape)
        ...
        # print('transformer 4:', image_features.shape, lidar_features.shape)
        """
        ...
        # print('fused feature:', fused_features.shape)

        embeds = self.embeddings(var_name)

        attn_weights, attn_outputs = self.attn(fused_features, embeds)

        attn_outputs = attn_outputs.view(image.shape[0], -1)

        var_value_o = F.relu(self.fc_l(var_value))

        inputs = torch.cat((torch.squeeze(fused_features), torch.squeeze(attn_outputs),
                            torch.mul(torch.squeeze(attn_weights), var_value_o)), 1)

        conv1_outputs = F.relu(self.conv1(inputs.unsqueeze(1)))
        conv2_outputs = F.relu(self.conv2(conv1_outputs))
        conv3_outputs = F.relu(self.conv3(conv2_outputs))
        conv4_outputs = F.relu(self.conv4(conv3_outputs))

        # print(conv4_outputs.shape)
        # print(conv4_outputs.view(-1, 32 * 98).shape)

        fc2_outputs = F.relu(self.fc2(conv4_outputs.view(-1, 32 * 98)))
        fc3_outputs = F.relu(self.fc3(fc2_outputs))
        fc4_outputs = F.relu(self.fc4(fc3_outputs))
        fc5_outputs = self.fc5(fc4_outputs)

        # print(fc5_outputs, fc5_outputs.shape)
...
class ImageCNN(nn.Module):
    ...
    def __init__(self, normalize=True):
        ...
        self.features = timm.create_model('regnety_016', pretrained=True)
...
class LidarEncoder(nn.Module):
    ...
    def __init__(self, in_channels=2):
        ...
        self._model = timm.create_model('regnety_016', pretrained=False)
        self._model.fc = None
...
class Attn:
    ...
    def __call__(self, source, target):
        self.weights = torch.sigmoid(torch.matmul(torch.unsqueeze(source, dim=1), torch.transpose(target, -2, -1)))
        self.outputs = torch.matmul(self.weights, target)

        return self.weights, self.outputs
