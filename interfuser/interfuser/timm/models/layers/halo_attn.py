...
class PosEmbedRel(nn.Module):
    ...
    def __init__(self, block_size, win_size, dim_head, scale):
        ...
        self.height_rel = nn.Parameter(
            torch.randn(win_size * 2 - 1, dim_head) * self.scale
        )
...
class HaloAttn(nn.Module):
    ...
    def __init__(
        ...
        # example_data in unfolded block form. I haven't wrapped my head around how that'd look.
        ...
        self.pos_embed = PosEmbedRel(
            block_size=block_size // self.stride,
    ...
    def forward(self, x):
        ...
        q = F.unfold(
            q,
            kernel_size=self.block_size // self.stride,
            stride=self.block_size // self.stride,
        )
        ...
        attn_logits = (
            q @ k.transpose(-1, -2)
        ) * self.scale  # FIXME should usual attn scale be applied?
        attn_logits = attn_logits + self.pos_embed(
            q
        )  # B * num_heads, block_size ** 2, win_size ** 2

        attn_out = attn_logits.softmax(dim=-1)
        attn_out = (attn_out @ v).transpose(
            1, 3
        )  # B * num_heads, dim_v // num_heads, block_size ** 2, num_blocks
        attn_out = F.fold(
