...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        #     print('/home/complexse/workspace/Chengjie/InterFuser/leaderboard/team_code/interfuser_agent.py')
        #     string = pathlib.Path(
        #         '/home/chengjielu/D1/projects/epiga-carla/epiga-project/leaderboard/example_data/42routes/42routes.xml').stem + "_"
        #
        #     string += "_".join(
        #         map(
        #             lambda x: "%02d" % x,
    ...
    def tick(self, input_data):
        ...
        if self.count_tick == 0:
            ...
            rgb_s = cv2.cvtColor(input_data["rgb_s"][1][:, :, :3], cv2.COLOR_BGR2RGB)
            rgb_left_s = cv2.cvtColor(input_data["rgb_left_s"][1][:, :, :3], cv2.COLOR_BGR2RGB)
            rgb_right_s = cv2.cvtColor(
                input_data["rgb_right_s"][1][:, :, :3], cv2.COLOR_BGR2RGB
            )
            rgb_a = np.concatenate([rgb_left_s, rgb_s, rgb_right_s], axis=1)
            img = cv2.cvtColor(rgb_a, cv2.COLOR_RGB2BGR)
            cv2.imwrite('./states/rgb_{}.png'.format(self.tag), img)
            ...
            birdview = birdview_producer.produce(
                agent_vehicle=CarlaDataProvider.get_hero_actor()  # carla.Actor (spawned vehicle)
            )
            bev = BirdViewProducer.as_rgb(birdview)
            # print(bev.shape)
            cv2.imwrite(str('./states/bev_{}.png'.format(self.tag)), bev)

        gps = input_data["gps"][1][:2]
        # print(input_data["speed"][1])
    ...
    def run_step(self, input_data, timestamp):
        ...
        # print((
        #             traffic_meta, traffic_meta.shape, '\n',
        #             pred_waypoints, pred_waypoints.shape, '\n',
        #             is_junction, is_junction.shape, '\n',
        #             traffic_light_state, traffic_light_state.shape, '\n',
        #             stop_sign, stop_sign.shape, '\n',
        #             bev_feature, bev_feature.shape, '\n'
        #         ))
        #
        # img = cv2.cvtColor(bev_feature.squeeze(0).cpu().numpy(), cv2.COLOR_RGB2BGR)
        # cv2.imwrite('./rgb_{}.png'.format('bev_feature'), img)
