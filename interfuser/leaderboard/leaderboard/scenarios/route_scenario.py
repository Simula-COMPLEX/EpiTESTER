...
class RouteScenario(BasicScenario):
    ...
    def _update_ego_vehicle(self):
        ...
        spectator.set_transform(carla.Transform(ego_trans.location + carla.Location(x=0, y=0, z=2),
                                ego_trans.rotation))
    ...
    @staticmethod
    def spawn_vehicles_around_ego_vehicles(radius, numbers_of_vehicles):
        # parameters:
        ...
        ego_location = CarlaDataProvider.get_hero_actor().get_location()
        accessible_points = []
        count = 0
        for spawn_point in spawn_points:
            dis = math.sqrt(
                (ego_location.x - spawn_point.location.x) ** 2 + (ego_location.y - spawn_point.location.y) ** 2)
            # it also can include z-coordinate,but it is unnecessary
            if dis < radius:
                # print(dis)
                count += 1
                accessible_points.append(spawn_point)
            if count >= numbers_of_vehicles:
                break
    ...
    def _initialize_actors(self, config):
        ...
        #     'Town08': 20,
        #     'Town09': 20,
        #     'Town10HD': 20,  # town10 doesn't load properly for some reason
        # }

        amount = town_amount[config.town] if config.town in town_amount else 0

        # way_points = self.spawn_vehicles_around_ego_vehicles(40, amount)
