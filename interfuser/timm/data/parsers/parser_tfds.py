...
class ParserTfds(Parser):
    ...
    def _lazy_init(self):
        ...
        if worker_info is not None:
            ...
            # FIXME I need to spend more time figuring out the best way to distribute/split example_data across
            ...
            if not self.is_training and "[" not in self.split:
                ...
                self.subsplit = even_split_indices(
                    self.split, global_num_workers, self.num_samples
        ...
        if self.shuffle:
            ds = ds.shuffle(
                min(self.num_samples, SHUFFLE_SIZE) // self._num_pipelines, seed=0
            )
    ...
    def __iter__(self):
        ...
        target_sample_count = math.ceil(
            max(1, self.repeats) * self.num_samples / self._num_pipelines
        )
        if self.is_training:
    ...
    def __len__(self):
        ...
        return math.ceil(
            max(1, self.repeats) * self.num_samples / self.dist_num_replicas
        )
