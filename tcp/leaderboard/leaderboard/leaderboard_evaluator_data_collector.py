...
from leaderboard.scenarios.scenario_manager_2 import ScenarioManager
...
PATH = '/home/complexse/workspace/Chengjie/transfuser'
...
class LeaderboardEvaluator(object):
    ...
    def _load_and_run_scenario(self, args, config, solution):
        """
        ...
        if args.scenario_id in ['scenario_3', 'scenario_4']:
            self.spawn_npc(vertical=16, horizontal=5, behavior=0.3, color='180, 44, 44')
            print(args.scenario_id)
        elif args.scenario_id in ['scenario_5']:
            self.spawn_npc(vertical=-10, horizontal=5, behavior=0.3, color='180, 44, 44')

        elif args.scenario_id in ['scenario_6']:
            self.span_road_debris(vertical=4, horizontal=0)
            self.span_road_debris(vertical=6, horizontal=0)
            self.span_road_debris(vertical=8, horizontal=0)

        # self.spawn_npc(vertical=solution[0], horizontal=solution[1], behavior=solution[2], color='44, 44, 44')
        # self.spawn_pedestrian(vertical=solution[3], horizontal=solution[4], direction_x=solution[5],
        #                       direction_y=solution[6], speed=solution[7])
        # self.configure_weather(sun_angle=solution[8], fog_density=solution[9])

        self.objective, self.min_dis = self.manager.run_scenario(args.scenario_id)
        self.tag = self.manager.tag
        self.elapsed_seconds = self.manager.elapsed_time
        self.ticks = self.manager.count_tick

        # print(CarlaDataProvider.get_hero_actor().get_transform())
    ...
    def configure_weather(self, sun_angle, fog_density):
        """
        sun angle: -0.9 - 0.9
        fog density: 0-1
        """
        weather = carla.WeatherParameters(sun_altitude_angle=sun_angle, fog_density=fog_density)
        # for vehicle in self.ego_vehicles:
        if sun_angle < 0:
            CarlaDataProvider.get_hero_actor().set_light_state(carla.VehicleLightState(self._vehicle_lights))
        self.world.set_weather(weather)

    def distance_constraints(self, wp):
    ...
    def spawn_npc(self, vertical: float, horizontal: float, behavior: float, color: str):
        """
        horizontal distance from the ego vehicle: [-20, 20]
        vertical distance from the ego vehicle: [-20, 20]
        behavior: [0, 0.3], switch lane (right), maintain lane, switch lane (left)
        """
        ...
        waypoint = self.world.get_map().get_waypoint(waypoint_t.location, project_to_road=True,
        ...
        while npc_vehicle is None:
            ...
            if try_spawn:
                try:
                    ...
                    return npc_vehicle
            # if try_spawn:
            #     npc_vehicle = self.world.try_spawn_actor(bp, npc_transform)
            try:
        ...
        if 0 <= behavior < 0.1:
            ...
            self.traffic_manager.force_lane_change(npc_vehicle, bool(0.1 <= behavior < 0.2))
        self.world.tick()
    ...
    def span_road_debris(self, vertical=-10, horizontal=5):
        ego_transform = CarlaDataProvider.get_hero_actor().get_transform()

        # print(ego_transform.location.z)
        # ego_transform.location.z = 1
        ...
        waypoint_t = self.world.get_map().get_waypoint(waypoint_t.location, project_to_road=True,
                                                       lane_type=carla.LaneType.Any)

        waypoint_t = carla.Transform(carla.Location(waypoint_t.transform.location.x,
                                                    waypoint_t.transform.location.y, waypoint_t.transform.location.z),
                                     waypoint_t.transform.rotation)

        first_debris = CarlaDataProvider.request_new_actor('static.prop.dirtdebris01', waypoint_t, 'prop')
        first_debris.set_transform(waypoint_t)
    ...
    def run(self, args):
        ...
        while route_indexer.peek():
            ...
            # run
            solution = [0.5818800439737557, 0.7145551981912506, 0.7684631691707429, 1, 0.6024137197394054,
                        0.17706739414285583,
                        0.16574693205485336, 0.07183908045977011, 0.7767982636547537, 0.47795659213522335]
            for i in range(2):
                self._load_and_run_scenario(args, config, solution=solution)
...
def main():
    ...
    parser.add_argument('--trafficManagerPort', default='3050',
...
if __name__ == '__main__':
    os.system('kill $(lsof -t -i:{})'.format(int(2000)))
    os.system('kill $(lsof -t -i:{})'.format(int(2050)))
    time.sleep(10)
    os.system('kill $(lsof -t -i:{})'.format(int(2000)))
    os.system('kill $(lsof -t -i:{})'.format(int(2050)))
    time.sleep(10)

    subprocess.Popen(['cd {}/carla/ && DISPLAY= ./CarlaUE4.sh --world-port={} -opengl'.format(PATH, int(2000))],
