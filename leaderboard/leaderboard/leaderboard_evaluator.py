...
PATH = '/home/complexse/workspace/Chengjie/InterFuser'
...
class LeaderboardEvaluator(object):
    ...
    def _load_and_run_scenario(self, args, config, solution):
        ...
        self.uncertainty_quantification = 'uncertainty_quantification_{}.csv'.format(timestamp)

        # pd.DataFrame([['steer', 'brake', 'throttle', 'junction', 'traffic_light', 'stop_sign',
        #                'junction_2', 'traffic_light_2', 'stop_sign_2']]).\
        #     to_csv('{}{}'.format(args.uq_path, self.uncertainty_quantification), mode='w', index=False, header=False)
        pd.DataFrame([['input_data', 'global_plan', 'timestamp']]).\
            to_csv('{}{}'.format(args.uq_path, self.uncertainty_quantification), mode='w', index=False, header=False)
        ...
        try:
            ...
            self._register_statistics(config, '{}{}'.format(args.checkpoint_path, self.checkpoint_file), entry_status, crash_message)
            self._cleanup()
            sys.exit(-1)
        ...
        self.objective, self.min_dis = self.manager.run_scenario(args.uq_path)
        self.tag = self.manager.tag
        self.elapsed_seconds = self.manager.elapsed_time
        self.ticks = self.manager.count_tick
        self.uq = self.manager.uq

        pd.DataFrame(self.uq).to_csv('{}{}'.format(args.uq_path, self.uncertainty_quantification), mode='a', index=False, header=False)
